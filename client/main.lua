...
RegisterCommand('banksReady', function(args, src, rawCommand)
	print("DEBUG: Registering command 'banksReady'.")
	...
	print("DEBUG: Banks are ready and Feather:Banks:Start triggered.")
end)
...
CreateThread(function()
    ...
    print("DEBUG: Initializing banks (auto instead of command).")
...
RegisterNetEvent('Feather:Banks:Start', function()
	print("DEBUG: Feather:Banks:Start event triggered.")
	BankOpen()
	BankClosed()
	...
	while (IsReady) do
		...
		for _, bank in pairs(Banks) do
			...
			if bank.open_hour and bank.close_hour then
				...
				print("DEBUG: Current hour: " .. hour)

				if hour >= bank.close_hour or hour < bank.open_hour then
					bank.isClosed = true
					print("DEBUG: Bank " .. bank.name .. " is CLOSED.")
				else
					if bank.isClosed then
						...
						print("DEBUG: Bank " .. bank.name .. " is OPEN now.")
					end
				end
			end
			...
			if bank.isClosed then
				if bank.blip_handle then
					if Config.BlipSettings.Show and Config.BlipSettings.ShowClosed then
						SetBlipColor(bank.blip_handle, bank.isClosed)
						print("DEBUG: Showing closed blip for " .. bank.name)
					else
						bank.blip_handle.Remove()
						bank.blip_handle = nil
						print("DEBUG: Removed closed blip for " .. bank.name)
					end
				end
				...
				if bank.npc then
					bank.npc:Remove()
					bank.npc = nil
					print("DEBUG: Removed NPC for closed bank " .. bank.name)
				end
			end
			...
			if Config.BlipSettings.Show then
				if Config.BlipSettings.UseDistance then
					if distance <= Config.BlipSettings.Distance then
						...
						if bank.blip_handle then
							...
							bank.blip_handle = nil
							print("DEBUG: Removed blip for bank " .. bank.name)
						end
					end
				else
					if bank.isClosed then
				...
				if bank.blip_handle then
					RemoveBlip(bank.blip_handle)
					bank.blip_handle = nil
					print("DEBUG: Removed blip for bank " .. bank.name .. " due to Config.BlipSettings.Show being false.")
				end
			end
			...
			if distance <= Config.PromptSettings.Distance then
				...
				if bank.isClosed then
					...
					print("DEBUG: Showing closed prompt for bank " .. bank.name)
				else
					GetOpenPromptGroup():ShowGroup(bank.name)
					if GetOpenPrompt():HasCompleted() then
						OpenUI(bank)
						print("DEBUG: Prompt completed, opening UI for bank " .. bank.name)
					end
				end
			end
		end
		...
		if sleep then
			print("DEBUG: Sleeping, no actions for banks.")
...
AddEventHandler('onResourceStop', function(resourceName)
	...
	print("DEBUG: Resource " .. resourceName .. " stopped. Cleaning up.")
	CleanUp()
end)
